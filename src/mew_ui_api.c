...
int MewUI_PlayMovieClipFrame(void* movie_clip, int32_t frame_index)
{
    ...
    __try
    {
        /* Native coinpulse callers do both operations: goto/play frame 1, then mark
           the display node dirty/active by setting bit 0x02 in the byte at +0x09. */
...
int MewUI_PlayMovieClipChildFrame(void* root_node, const char* child_name, int32_t frame_index)
{
    void* movie_clip;
    ...
    movie_clip = MewUI_FindChildByName(root_node, child_name);
    if (!movie_clip)
    {
        movie_clip = MewUI_FindNodeByPathOrName(root_node, child_name, 0U);
    }
    if (!movie_clip)
    {
        movie_clip = MewUI_FindNodeByPathOrName(root_node, child_name, 1U);
    }
